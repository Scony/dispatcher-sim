...
void SASADispatcher::operationLevelSA()
{
  auto costFunction = [&](const std::map<long long, std::vector<OperationSP>>& solution) {
    ...
    for (auto const& jobId : mCurrentSolution)
      opPermutation.insert(
          opPermutation.end(), solution.at(jobId).begin(), solution.at(jobId).end());
  ...
  mJobOperations = algorithm::sa<std::map<long long, std::vector<OperationSP>>, long long>(
      mJobOperations, costFunction, neighbouringSolution, mIterations);
}
