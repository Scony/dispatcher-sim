...
void JSADispatcher::dispatch(std::shared_ptr<Job> job)
{
  ...
  auto costFunction = [&](const std::vector<long long>& solution) {
    ...
    for (auto const& jobId : solution)
      opPermutation.insert(
          opPermutation.end(), mJobOperations[jobId].begin(), mJobOperations[jobId].end());
  ...
  mCurrentSolution = algorithm::sa<std::vector<long long>, long long>(
      mCurrentSolution, costFunction, neighbouringSolution, mIterations);
}
