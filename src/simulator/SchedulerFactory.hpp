...
class SchedulerFactory
{
 public:
  SchedulerFactory(
      ...
      : mInput(input), mMachines(machines), mEstimator(estimator), mArguments(arguments)
  {
  }
...
std::shared_ptr<Scheduler<Schedule>> SchedulerFactory<Schedule>::create()
{
  ...
  if (mArguments.primaryAlgorithm == "random")
    scheduler = std::make_shared<RandomScheduler>(mInput, mMachines, mEstimator);
  if (mArguments.primaryAlgorithm == "sa")
    scheduler =
        std::make_shared<SAScheduler>(mInput, mMachines, mEstimator, mArguments.saIterations);
  if (mArguments.primaryAlgorithm == "sj")
    scheduler = std::make_shared<SJScheduler>(mInput, mMachines, mEstimator);
  if (mArguments.primaryAlgorithm == "sjlo")
    scheduler = std::make_shared<SJLOScheduler>(mInput, mMachines, mEstimator);
...
std::shared_ptr<Scheduler<CapacitySchedule>> SchedulerFactory<CapacitySchedule>::create()
{
  ...
  if (mArguments.primaryAlgorithm == "random")
    scheduler = std::make_shared<RandomCapacityScheduler>(mInput, mMachines, mEstimator);
  if (mArguments.primaryAlgorithm == "fifo")
    scheduler = std::make_shared<FIFOCapacityScheduler>(mInput, mMachines, mEstimator);
  if (mArguments.primaryAlgorithm == "sj")
    scheduler = std::make_shared<SJCapacityScheduler>(mInput, mMachines, mEstimator);
  if (mArguments.primaryAlgorithm == "sjlo")
    scheduler = std::make_shared<SJLOCapacityScheduler>(mInput, mMachines, mEstimator);
  if (mArguments.primaryAlgorithm == "sa")
    scheduler = std::make_shared<SACapacityScheduler>(
        mInput, mMachines, mEstimator, mArguments.saIterations);
  assert(scheduler != nullptr && "Cannot create such algorithm for this representation");
  return scheduler;
}
